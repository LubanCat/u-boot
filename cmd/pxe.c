...
static void label_boot_fdtoverlay(cmd_tbl_t *cmdtp, struct pxe_label *label)
{
	...
	fdtoverlay_addr_env = env_get("fdt_over_addr");
	if (!fdtoverlay_addr_env) {
		printf("Invalid fdt_over_addr for loading overlays\n");
		return;
	}
...
static int parse_label(char **c, struct pxe_menu *cfg)
{
	...
	while (1) {
		...
		case T_DTOVERLAY:
			if (!label->dtoverlay)
				err = parse_sliteral(c, &label->dtoverlay);
			break;
