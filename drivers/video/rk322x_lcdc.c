			    ...
			    vidinfo_t *vid)
{
	...
	} else if (vop_dev->overlay_mode == VOP_RGB_DOMAIN) {
		switch (fb_info->format) {
		case YUV420:
...
static int vop_win_0_1_reg_update(struct vop_device *vop_dev,
...
static int win0_set_par(struct vop_device *vop_dev,
			struct fb_dsp_info *fb_info,
			vidinfo_t *vid)
{
	uint64_t val;
	...
	win.csc_mode = vop_vop_csc_mode(vop_dev, fb_info, vid);
	if (fb_info->yaddr)
		win.state = 1;
	else
		win.state = 0;
...
void rk_lcdc_set_par(struct fb_dsp_info *fb_info, vidinfo_t *vid)
{
	...
	case WIN0:
	case WIN1:
		win0_set_par(vop_dev, fb_info, vid);
		break;
	default:
...
static void vop_bcsh_path_sel(struct vop_device *vop_dev)
{
	...
	if (vop_dev->overlay_mode == VOP_YUV_DOMAIN) {
		...
		if (vop_dev->output_color == COLOR_RGB) {
...
int rk_lcdc_load_screen(vidinfo_t *vid)
{
	...
	case SCREEN_TVOUT:
		...
		if (screen->mode.xres == 720 && screen->mode.yres == 576)
			val |= V_SW_TVE_MODE(1);
		else
			val |= V_SW_TVE_MODE(0);
...
int vop_vop_read_def_cfg(struct vop_device *vop_dev)
{
	int reg = 0;
...
int rk_lcdc_init(int vop_id)
{
	...
	vop_writel(vop_dev, CABC_GAUSS_LINE0_0, 0x15110903);
	vop_writel(vop_dev, CABC_GAUSS_LINE0_1, 0x00030911);
	vop_writel(vop_dev, CABC_GAUSS_LINE1_0, 0x1a150b04);
	vop_writel(vop_dev, CABC_GAUSS_LINE1_1, 0x00040b15);
	vop_writel(vop_dev, CABC_GAUSS_LINE2_0, 0x15110903);
	vop_writel(vop_dev, CABC_GAUSS_LINE2_1, 0x00030911);
